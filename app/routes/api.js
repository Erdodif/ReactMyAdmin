...
export default function api(api){
    ...
    api.post('/:databases/query', (req, res) => {
        try {
            let database = req.params["database"];
            let sql = req.body["sql"];
            connectToDatabase(database)
    ...
    api.get('/databases/:database', (req, res) => { res.sendStatus(200) });

    api.get('/databases/:database/tables', (req, res) => {
        try {
            let database = req.params["database"];
            connectToDatabase(database)
                .then(() => {
    ...
    api.get('/databases/:database/tables/:table', (req, res, next) => {
        try {
            let database = req.params["database"];
            let table = req.params["table"];
            connectToDatabase(database)
